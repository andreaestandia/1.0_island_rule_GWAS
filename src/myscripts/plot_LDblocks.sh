...
# This script was modified by Andrea Estandia 
# but it is based on https://github.com/fgvieira/ngsLD/blob/master/scripts/LD_blocks.sh written by Filipe G. Vieira
...
for (ld in c("r2","Dp")) {
  ...
  # Save plot
  pdf(paste0("LD_blocks_", ld,"_", start, ".pdf"), width=10, height=10)
  LDheatmap(m, genetic.distances=dist, SNP.name=id, geneMapLabelX=0.75, geneMapLabelY=0.25, color=pal, LDmeasure=ld)
  x <- dev.off()
}
EOF
