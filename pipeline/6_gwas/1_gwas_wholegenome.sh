...
do
	nohup $PATH_ANGSD -doMaf 4 -beagle "${PATH_GENOME}${trait}.beagle.gz" -doAsso 4 -yQuant "${PATH_PHENOTYPES}${trait}.tsv" -out "${PATH_OUT}${trait}_gwas_wholegenome" -fai $PATH_FAI -cov "${PATH_COV}${trait}_control" -Pvalue 1 &
done
