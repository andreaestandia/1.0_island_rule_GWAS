...
samtools mpileup -Bx -uf "${PATH_REF}GCA_007252995.1_ZoBo_15179_v2.0_genomic.fna" "${PATH_INPUT}ZoBo_sort_hisat.bam" | bcftools call -c | vcfutils.pl vcf2fq > "${PATH_INPUT}ZoBo_transcriptome.fastq"
#"${PATH_TRINITY}Trinity" --genome_guided_bam "${PATH_INPUT}ZoBo_sort_hisat.bam" --genome_guided_max_intron 100000 --output "${PATH_OUT}" --max_memory 150G --CPU 20 > "${PATH_OUT}trinity.log.out"
