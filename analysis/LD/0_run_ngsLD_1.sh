...
chromosome_list+=("chr20")

for chromosome in "${chromosome_list[@]}"
do
	N_SITES=$(cat "${PATH_INPUT}${chromosome}.beagle_0${SLURM_ARRAY_TASK_ID}.gz_pos" | wc -l)
	#N_SITES=$((tmp-1))
	ngsLD --geno "${PATH_INPUT}${chromosome}.beagle_0${SLURM_ARRAY_TASK_ID}.gz" --probs --n_ind  $N_IND  --n_sites $N_SITES --out "${PATH_OUTPUT}${chromosome}_0${SLURM_ARRAY_TASK_ID}.ld" --n_threads $N_THREADS --max_kb_dist 10 --extend_out --pos "${PATH_INPUT}${chromosome}.beagle_0${SLURM_ARRAY_TASK_ID}.gz_pos"
done
