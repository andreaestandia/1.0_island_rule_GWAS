...
chromosome_list+=("chr9")

for chromosome in "${chromosome_list[@]}"
do
        perl $PATH_PRUNE --in_file "${PATH_INPUT}${chromosome}_0${SLURM_ARRAY_TASK_ID}.ld" --max_kb_dist $MAX_KB_DIST --min_weight $MIN_WEIGHT --out "${PATH_OUT}${chromosome}_0${SLURM_ARRAY_TASK_ID}_pruned.ld"
done
