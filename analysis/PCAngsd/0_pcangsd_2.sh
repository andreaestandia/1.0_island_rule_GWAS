...
do
       #nohup  gzip -c "${PATH_BEAGLE}${trait}_chr2.beagle" > "${PATH_BEAGLE}${trait}_chr2.beagle.gz" &
       nohup python "${PATH_PCANGSD}pcangsd.py" -beagle "${PATH_BEAGLE}${trait}_chr2.beagle.gz" -admix -dosage_save -out "${PATH_OUT}${trait}_pca_out" -threads $N_THREADS &
done
