...
do
	nohup  gzip -c "${PATH_BEAGLE}${trait}.beagle" > "${PATH_BEAGLE}${trait}.beagle.gz" &
	python "${PATH_PCANGSD}pcangsd.py" -beagle "${PATH_BEAGLE}${trait}.beagle.gz" -admix -dosage_save -out "${PATH_OUT}${trait}_pca_out" -threads $N_THREADS
done
