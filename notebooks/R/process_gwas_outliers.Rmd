...
outlier <- 
  read.table(file.path(reports_path, 
                       "GWAS", 
                        "outliers",
                        "body_pc1_outliers.beagle"),sep="\t", header=T) %>% 
...
outliers_chr2 <- 
  read_tsv(file.path(reports_path, 
                       "annotation", 
                        "outliers",
                        "outliers_body_PC1.xml"),col_names = F) %>% 
...
for (chr in as.character(top_chr$Chromosome)) {
  t <- info_outliers %>%
    filter(Chromosome == chr) %>%
    filter(-log10(P) > -log10(0.01 / nrow(info_outliers))) %>%
    arrange(P) %>%
    filter(Position < median(Position) + 10000) %>%
    filter(Position > median(Position) - 10000)
  
  df <-
    data.frame(
      start_position = min(t$Position),
      end_position = max(t$Position),
      chromosome_name = paste0("chromosome", chr)
    )
...
write.table(df_total, 
            file.path(reports_path,
                      ...
                      "outliers",
                      "annotation",
                      "positions4annotation_billPC1.csv"), row.names = F,
            sep="\t", quote = FALSE)
```
