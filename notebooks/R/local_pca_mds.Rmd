...
big_data <- 
  read.csv(file.path(reports_path,
                       "localPCA/unpruned/chrZ_big_data.csv"),
             header=T)
...
big_data %>%
  ...
  theme(
    ...
    panel.border = element_blank())+gghighlight::gghighlight(PC1 < -sd(big_data$PC1)*4)
...
```{r}
cov_mat <- as.matrix(read.table("/media/sjoh4959/My Passport/Andrea/projects/1.0_island_rule_GWAS/reports/localPCA/outlier_mds/cov/chr30_1452719-chr30_1651499.cov"))
