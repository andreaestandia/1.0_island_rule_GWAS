...
big_data <- read.table(file.path(reports_path,"localPCA/big_data/chr1-30Z4A1A_localpca.csv"), header=T)
chr30 <- big_data %>% filter(LG=="chr30")
...
mds_by_chr <- function(chr){
  ...
  for (i in 1: dim(window_order)[1])
  {
    ...
    window_covmats_i<-as.matrix(read.table(file.path(reports_path, paste0("localPCA/cov_by_window/",chr,"/",window_order$file_name[i]))))
...
```{r}
cov_mat <- as.matrix(read.table(file.path(reports_path, paste0("localPCA/chr30_inversion/inversions_chr30_pca_out.cov"))))

pheno <- read_csv("/home/zoo/sjoh4959/Documents/projects/0.0_island_rule/data/raw/phenotypes_gwas.csv")
...
pca.out %>% 
  ...
  labs(x="\nPC1", y="PC2\n", title="Inversion in Chromosome 30", family="ubuntu")
